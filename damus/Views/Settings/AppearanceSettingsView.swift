        ...
        Form {
            ...
            Section(header: Text(NSLocalizedString("Accessibility", comment: "Section header for accessibility settings"))) {
                Toggle(NSLocalizedString("Left handed", comment: "Moves the post button to the left side of the screen"), isOn: $settings.left_handed)
                    .toggleStyle(.switch)
                Toggle(NSLocalizedString("Capitalise hashtags", comment: "Capitalises the first letter of each word in a recognised hashtag, e.g. changes #plebchain to #PlebChain"), isOn: $settings.capitalize_hashtags)
                    .toggleStyle(.switch)
            }
