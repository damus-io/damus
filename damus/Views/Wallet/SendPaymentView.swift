...
struct SendPaymentView: View {
    ...
    enum SendState {
        ...
        case completed
        case failed(HumanReadableError)
    }
    ...
    @State private var sendState: SendState = .enterInvoice(scannerMessage: nil) {
        didSet {
            switch sendState {
            case .enterInvoice, .confirmPayment, .processingPayment:
                break
            case .completed:
            ...
            VStack(spacing: 15) {
                Button(action: {
                    if let pastedInvoice = getPasteboardContent() {
                        processInvoiceString(pastedInvoice)
                    }
                }) {
                    HStack {
                            ...
                            if let error = error as? WalletModel.WaitError {
                                ...
                                case .timeout:
                                    sendState = .failed(.init(
                                        ...
                                        tip: NSLocalizedString("Check if your wallet is online, configured correctly, try again. If the error persists, please contact support and/or your wallet provider.", comment: "A human-readable tip guiding the user on what to do when seeing a timeout error while sending a wallet payment."),
    ...
    func handleScan(result: Result<ScanResult, ScanError>) {
        ...
        case .success(let result):
            processInvoiceString(result.string)
    ...
    func processInvoiceString(_ text: String) {
        var processedInvoiceString = text.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        
        guard let invoice = Invoice.from(string: processedInvoiceString) else {
            sendState = .enterInvoice(scannerMessage: NSLocalizedString("This does not appear to be a valid Lightning invoice.", comment: "A user-readable message indicating that the lightning invoice they scanned or pasted was not valid."))
            return
        }
        guard invoice.amount != .any else {
