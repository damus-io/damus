...
struct RelayDetailView: View {
    ...
    func check_connection() -> Bool {
        ...
        return state.networkManager.userRelayList.getUserCurrentRelayList()?.relays.keys.contains(self.relay) == true
    }
    ...
    private var relay_object: RelayPool.Relay? {
        ...
        state.networkManager.connectedRelays.first(where: { $0.descriptor.url == relay })
    }
    ...
    func removeRelay() async {
        do {
            // TODO: Concurrency problems?
            try await state.networkManager.userRelayList.remove(relayURL: self.relay)
            dismiss()
        }
        catch {
