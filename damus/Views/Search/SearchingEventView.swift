...
struct SearchingEventView: View {
    ...
    func handle_search(search: SearchType) {
        ...
        case .event(let note_id):
            Task {
                let res = await state.networkManager.findEvent(query: .event(evid: note_id))
        ...
        case .profile(let pubkey):
            Task {
                let res = await state.networkManager.findEvent(query: .profile(pubkey: pubkey))
        ...
        case .naddr(let naddr):
            Task {
                let res = await state.networkManager.lookup(naddr: naddr)
