            ...
            if let meta = state.events.lookup_img_metadata(url: url),
               case .processed(let blurhash) = meta.state {
                Image(uiImage: blurhash)
                    .resizable()
                    .frame(maxWidth: preview ? 350 : UIScreen.main.bounds.width, minHeight: preview ? 150 : 200, maxHeight: preview ? 150 : 200)
            } else {
...
struct FollowPackPreviewBody: View {
    ...
    var Main: some View {
        ...
        .frame(width: 350, height: state.settings.media_previews ? 300 : 150, alignment: .leading)
