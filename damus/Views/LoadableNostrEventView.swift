...
class LoadableNostrEventViewModel: ObservableObject {
    ...
    private func executeLoadingLogic() async -> ThreadModelLoadingState {
        ...
        case .note_id(let note_id):
            ...
            switch known_kind {
            case .text:
                return .loaded(route: Route.Thread(thread: await ThreadModel(event: ev, damus_state: damus_state)))
            case .dm:
                let dm_model = DirectMessageModel(our_pubkey: damus_state.pubkey, pubkey: ev.pubkey)
            ...
            case .like:
                ...
                guard let liked_ev = await self.loadEvent(noteId: first_referenced_note_id) else { return .not_found }
                return .loaded(route: Route.Thread(thread: await ThreadModel(event: liked_ev, damus_state: damus_state)))
            case .contacts, .metadata, .delete, .boost, .chat, .mute_list, .list_deprecated, .draft, .longform, .zap, .zap_request, .highlight, .nwc_request, .nwc_response, .http_auth, .status:
                return .unknown_or_unsupported_kind
            }
        case .naddr(let naddr):
            guard let event = await naddrLookup(damus_state: damus_state, naddr: naddr) else { return .not_found }
            return .loaded(route: Route.Thread(thread: await ThreadModel(event: event, damus_state: damus_state)))
        }
    }
