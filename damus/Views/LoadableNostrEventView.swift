...
class LoadableNostrEventViewModel: ObservableObject {
    ...
    private func loadEvent(noteId: NoteId) async -> NostrEvent? {
        let res = await damus_state.networkManager.findEvent(query: .event(evid: noteId))
    ...
    private func executeLoadingLogic(note_reference: NoteReference) async -> ThreadModelLoadingState {
        ...
        case .note_id(let note_id):
            ...
            case .contacts, .metadata, .delete, .boost, .chat, .mute_list, .list_deprecated, .draft, .longform, .zap, .zap_request, .nwc_request, .nwc_response, .http_auth, .status, .relay_list:
                return .unknown_or_unsupported_kind
            }
        case .naddr(let naddr):
            guard let event = await damus_state.networkManager.lookup(naddr: naddr) else { return .not_found }
