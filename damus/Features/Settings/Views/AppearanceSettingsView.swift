...
private enum CacheClearingState: Equatable {
    case not_cleared
    case clearing(phase: CacheClearingPhase)
    case cleared
...
struct AppearanceSettingsView: View {
    ...
    var body: some View {
        ZStack {
            ...
            .disabled(cache_clearing_state.isClearing)

            // Full-screen modal overlay during cache clearing
            CacheClearingOverlay
        }
        .animation(.easeInOut(duration: 0.2), value: cache_clearing_state)
        .navigationTitle(NSLocalizedString("Appearance", comment: "Navigation title for text and appearance settings."))
        .navigationBarBackButtonHidden(cache_clearing_state.isClearing)
        .interactiveDismissDisabled(cache_clearing_state.isClearing)
        .onReceive(handle_notify(.switched_timeline)) { _ in
            // Don't dismiss if cache is being cleared
            guard !cache_clearing_state.isClearing else { return }
            dismiss()
        }
    }
    
    /// Triggers the phased cache clear and manages UI state timing.
    func clear_cache_button_action() {
        cache_clearing_state = .clearing(phase: .kingfisher)
        ...
        DamusCacheManager.shared.clear_cache(
            ...
            onPhaseChange: { phase in
                self.cache_clearing_state = .clearing(phase: phase)
            },
            completion: {
                group.leave()
            }
    ...
    /// Full-screen modal overlay that blocks all interaction while cache is being cleared.
    @ViewBuilder
    var CacheClearingOverlay: some View {
        if case .clearing(let phase) = cache_clearing_state {
            ZStack {
