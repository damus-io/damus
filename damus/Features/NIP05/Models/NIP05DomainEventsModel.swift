...
class NIP05DomainEventsModel: ObservableObject {
    ...
    nonisolated private func resolve_domain_match(pubkey: Pubkey) async -> Bool {
        ...
        // Use defer to ensure cleanup even if cancelled or throws
        defer {
            Task { @MainActor in
        ...
        return await MainActor.run { matches_domain(pubkey) }
    }
    ...
    func streamItems() async {
        ...
        let authors = NIP05DomainHelpers.authors_for_domain(
            domain: domain,
