...
struct ContentView: View {
    ...
    func MainContent(damus: DamusState) -> some View {
        VStack {
            ...
            case .home:
                PostingTimelineView(damus_state: damus_state!, home: home, isSideBarOpened: $isSideBarOpened, active_sheet: $active_sheet, headerOffset: $headerOffset, tabHeight: $tabHeight)
            ...
            if let damus = self.damus_state {
                NavigationStack(path: $navigationCoordinator.path) {
                    ...
                    .overlay(alignment: .bottom) {
                        if !hide_bar {
                            TabBar(nstatus: home.notification_status, selected: $selected_timeline, headerOffset: $headerOffset, settings: damus.settings, action: switch_timeline)
                                .padding([.bottom], 8)
                                .background(selected_timeline != .home ? DamusColors.adaptableWhite : DamusColors.adaptableWhite.opacity(0.90 - (abs(headerOffset/100.0))))
