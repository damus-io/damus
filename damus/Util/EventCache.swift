...
func preload_event(plan: PreloadPlan, state: DamusState) async {
    ...
    if plan.load_preview, note_artifact_is_separated(kind: plan.event.known_kind) {
        let maybe_arts = await ContentRenderer().render_note_content(ndb: state.ndb, ev: plan.event, profiles: profiles, keypair: our_keypair)
        let arts = artifacts ?? maybe_arts
