...
class Ndb {
    ...
    /// Compacts the database, keeping all profiles and only notes authored by the given pubkeys.
    /// The compacted database is written to `output_path`.
        ...
        return try withNdb({
            ...
            let result = output_path.withCString { path_cstr in
                raw_pubkeys.withUnsafeBufferPointer { buf -> Int32 in
                    ...
                    // C expects `const unsigned char (*)[32]` — a pointer to
                    // arrays of 32 bytes. Our flat buffer is laid out identically
                    // so we just reinterpret the pointer.
    ...
    static func try_swap_compacted_db() -> Bool {
        ...
        do {
            ...
            // Remove the compact directory and its lock file
