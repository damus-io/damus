...
extension NostrNetworkManager {
    ...
    class UserRelayListManager: NWCManager.Delegate {
        ...
        private func getLatestNIP65RelayList() throws(LoadingError) -> NIP65.RelayList? {
            ...
            guard let latestRelayListEvent = delegate.ndb.lookup_note(latestRelayListEventId)?.unsafeUnownedValue?.to_owned() else { return nil }
        ...
        // MARK: - NWCManager delegate
        
        func nwcWalletChanged() {
            self.load()     // Reload relays in relay pool to include new NWC relay
        }
    }
}
