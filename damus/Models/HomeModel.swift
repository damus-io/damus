...
func process_local_notification(damus_state: DamusState, event ev: NostrEvent) {
    ...
    if type == .text && damus_state.settings.mention_notification {
        ...
        for case .mention(let mention) in blocks where mention.ref.ref_id == damus_state.keypair.pubkey {
            let content = NSAttributedString(render_note_content(ev: ev, profiles: damus_state.profiles, privkey: damus_state.keypair.privkey, camel_case_hashtags: damus_state.settings.capitalize_hashtags).content.attributed).string
