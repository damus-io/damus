...
func translate_note(profiles: Profiles, privkey: String?, event: NostrEvent, settings: UserSettingsStore, note_lang: String) async -> TranslateStatus {
    ...
    let artifacts = render_blocks(blocks: translated_blocks, profiles: profiles, privkey: privkey, camel_case_hashtags: UserSettingsStore.shared?.capitalize_hashtags ?? true)
