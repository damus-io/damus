...
//  damus
//
//  ThumbHash implementation from https://github.com/evanw/thumbhash
//  Author: Evan Wallace (https://evanw.github.io/thumbhash/)
//  License: MIT
//
//  ThumbHash is a compact representation of an image placeholder.
//  Compared to BlurHash, it offers:
//  - Better detail preservation
//  - Aspect ratio information embedded in the hash
//  - Alpha channel (transparency) support
//  - More accurate color representation
//
//  The hash is typically 25 bytes (base64: ~33 chars), similar to BlurHash.
//
...
/// Encodes RGBA pixel data into a ThumbHash.
///
/// - Parameters:
///   - w: Image width (must be <= 100)
///   - h: Image height (must be <= 100)
///   - rgba: Raw RGBA pixel data (w * h * 4 bytes)
/// - Returns: ThumbHash data (~25 bytes)
...
/// Decodes a ThumbHash into RGBA pixel data.
///
/// - Parameter hash: ThumbHash data
/// - Returns: Tuple of (width, height, rgba pixel data)
...
/// Extracts the average RGBA color from a ThumbHash without fully decoding it.
///
/// - Parameter hash: ThumbHash data
/// - Returns: Tuple of (r, g, b, a) values in range 0-1
...
/// Extracts the approximate aspect ratio from a ThumbHash without decoding it.
///
/// - Parameter hash: ThumbHash data
...
// MARK: - iOS UIImage Extensions

#if os(iOS)
import UIKit

/// Encodes a UIImage into a ThumbHash.
///
/// The image is first scaled down to max 100x100 while preserving aspect ratio,
/// then encoded using DCT (Discrete Cosine Transform) into a compact hash.
///
/// - Parameter image: Source UIImage to encode
/// - Returns: ThumbHash data (~25 bytes, suitable for base64 encoding)
func imageToThumbHash(image: UIImage) -> Data {
    ...
    var rgba = Data(count: w * h * 4)
    rgba.withUnsafeMutableBytes { rgba in
        guard let space = image.cgImage?.colorSpace else { return }
...
/// Decodes a ThumbHash into a UIImage placeholder.
///
/// The resulting image is typically around 32x32 pixels (aspect ratio preserved)
/// and is suitable for display as a blurred placeholder while the full image loads.
///
/// - Parameter hash: ThumbHash data
/// - Returns: Decoded UIImage placeholder
...
// MARK: - macOS NSImage Extensions

#if os(macOS)
import Cocoa

/// Encodes an NSImage into a ThumbHash.
///
/// - Parameter image: Source NSImage to encode
/// - Returns: ThumbHash data (~25 bytes)
func imageToThumbHash(image: NSImage) -> Data {
    ...
    rgba.withUnsafeMutableBytes { rgba in
        ...
        guard let space = (image.representations[0] as? NSBitmapImageRep)?.colorSpace.cgColorSpace else { return }
        guard let context = CGContext(
            data: rgba.baseAddress,
            width: w,
            height: h,
...
/// Decodes a ThumbHash into an NSImage placeholder.
///
/// - Parameter hash: ThumbHash data
/// - Returns: Decoded NSImage placeholder
