...
enum Route: Hashable {
    ...
    func view(navigationCoordinator: NavigationCoordinator, damusState: DamusState) -> AnyView {
        ...
        case .StorageSettings(settings: let settings):
            AnyView(StorageSettingsView(damus_state: damusState, settings: settings))
        case .NostrDBStorageDetail(stats: let stats):
            AnyView(NostrDBDetailView(damus_state: damusState, settings: damusState.settings, stats: stats))
