...
if ! error_output=$(sentry-cli debug-files upload --include-sources "${DSYM_PATH}" 2>&1 1>/dev/null); then
  echo "warning: sentry-cli failed to upload dSYMs with sources from ${DSYM_PATH}: ${error_output}"
  if ! error_output=$(sentry-cli debug-files upload "${DSYM_PATH}" 2>&1 1>/dev/null); then
